...
$.getJSON('../member/detail.json', {'mno': mno}, function(result) {	
    ...
    var generatedHTML = templateFn(result.data.id) // 템플릿 함수에 데이터를 넣고 HTML을 생성한다.
    myinfoBox.text('') //tbody의 기존 tr 태그들을 지우고
    myinfoBox.html(generatedHTML) // 새 tr 태그들로 설정한다.
    console.log("updateMember:" + result.data.loginType)
    if (result.data.loginType != null) {
    	$('#social-hidden1').css('display','none')
    	$('#social-hidden2').css('display','none')
