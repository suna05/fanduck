	...
	$('#add-btn').unbind('click').on('click', function(e) {
		...
		console.log(fiMpNo.val())
			...
			$.post('/board/add.json', {
				...
				console.log("2번째 파일", fiList2.val())
				console.log("3번째 파일", fiList3.val())
...
function(isConfirm){
	  if (isConfirm) {
	    ...
	    swal("삭제되었습니다.", "사진을 삭제했습니다.", "success");   
	  	console.log("bdList1", fiList1.val())
	  } else {
	    swal("취소", "사진 삭제 취소", "error");
	  }
	  $('#list1').empty('')
	});
...
function(isConfirm){
	  if (isConfirm) {
	    ...
	    swal("취소", "Your imaginary file is safe :)", "error");
	  }
	  $('#list2').empty('')
	});
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList1.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
...
function displayPhotoList2() {
  ...
  var newEl = ''
     newEl += [
    	 '<a id="pop2">' +
    	 '<div onclick=popup()>' +
        '<img src="/web/board/photo/'+ fiList2.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
        '</div>' +
        '</a>'
     ].join('')
  $target.html(newEl)
...
function displayPhotoList3() {
  ...
  var newEl = ''
     newEl += [
    	 '<a id="pop3">' +
    	 '<div onclick=popup()>' +
        '<img src="/web/board/photo/'+ fiList3.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
        '</div>' +
        '</a>'
     ].join('')
  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop4">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList4.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop5">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList5.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop6">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList6.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop7">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList7.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop8">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList8.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var newEl = ''
		     newEl += [
		    	 '<a id="pop9">' +
		    	 '<div onclick=popup()>' +
		        '<img src="/web/board/photo/'+ fiList9.val()+'.png" style="max-width: 100%; cursor:pointer;" >' +
		        '</div>' +
		        '</a>'
		     ].join('')
		  $target.html(newEl)
