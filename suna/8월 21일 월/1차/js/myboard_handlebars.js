...
function displayList1() {
  // 서버에서 사진 목록 데이터를 받아 온다.
	 ...
	 $.getJSON('/board/detail.json',{'bdNo': bdNo},function(result) {
		  ...
		  var $target = $('#list1-photo')
		  var newEl = ''
		     newEl += [
		        '<img src="/web/board/photo/'+ fiList1.val() +'.png" style="height: 240px; max-width: 90%;">'
		     ].join('')
		  $target.html(newEl)
...
function displayList2() {
  ...
  var $target = $('#list2-photo')
  var newEl = ''
     newEl += [
        '<img src="/web/board/photo/'+ fiList2.val() +'.png" style="height: 240px; max-width: 90%;">'
     ].join('')
  $target.html(newEl)
