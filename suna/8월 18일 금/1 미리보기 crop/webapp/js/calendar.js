...
//  console.log(Calendar.getDay()); // 2017년 7월 1일은 토요일이므로 숫자 '6'이 출력된다.
  // template calendar
  html += '<div class="prevBtn moveBtn" style="float:left; margin-left:10%; color:white; cursor:pointer">이전</div>';
  html += '<div class="nextBtn moveBtn" style="float:right; margin-right:10%; color:white; cursor:pointer">다음</div>';
  html += '<div class="head_cal" style="text-align:center">' + month_of_year[month] + '</div>';
  html += '<table>';
  // head
  html += '<thead>';
  html += '<tr class="subhead_cal"><th class="cal_year" colspan="7" style="text-align:center">' + year + '</th></tr>';
    ...
    if (Calendar.getDate() > index) {
      ...
      if (week_day !== 7) { // 일요일을 제외한 모든 요일에 테이블 데이터를 출력한다.
        ...
        if (today === Calendar.getDate() && currMonth === Calendar.getMonth()) {
          ...
          html += '<td class="'+ day +'"><a class="'+ day +'"href="#" data-id="' + info + '" onclick="showInfo(\'' + info + '\')">' +
            	  day + '</a></td>';	
        }
