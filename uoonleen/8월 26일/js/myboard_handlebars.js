...
$(document.body).on('click', '.detail-link', function(event) {
	location.href='board.html?mpNo=' + $(this).attr('data-no')
...
$.getJSON('/board/searchList.json', {
		...
		console.log(result.data.searchList.length)
		...
		else if (result.data.searchList.mpNo != mpNo || result.data.searchList.mpNo == '') {
			console.log(mpNo)
			console.log(result.data.searchList.mpNo)
...
if (bdNo == 0) { // 새 게시글 등록
	...
	$('#add-btn').unbind('click').on('click', function(e) {
		if (fiPhotoName.val() =='') {
			swal("등록 실패", "대표사진은 반드시 등록해주세요", "error")
			return;
			
		} else if(fiTitle.val() ==''){
			swal("등록 실패", "제목을 입력해주세요", "error")
			return;
