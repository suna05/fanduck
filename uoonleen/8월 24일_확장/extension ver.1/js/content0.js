        	  ...
        	  $.getJSON('http://localhost:8080/movie/list.json', function(result) {
                ...
                alert('movie list')
            		if (movieList.length < 1) {
          			  ...
          			  }, function(result) { alert('movie first add') }, 'json')
                       ...
                       alert('movie plus add')
                    }, 'json')
        	  			} // 개봉영화 db에 저장. 단, 테이블에 중복된 데이터가 없는 경우에.
      		      }

                if (movieCodeArr[0] != null) {
                  alert('movieCodeArr[0] != null')
                  $.post('http://localhost:8080/castmember/add.json', {
                    ...
                    'ifRead': false
                     ...
                     alert('castmember state')
                      var message = peopleNm + ' 님이 참여하신 ' + movieCodeArr[1] + ' 영화가 ' + movieCodeArr[2] + '에 개봉합니다!'
                      var options = {
                        type : 'basic',
                        title : '영화인 개봉 알림! ',
                        message : message,
