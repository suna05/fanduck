...
$('#upload-btn').click(function(e) {
    if (personTags.val().length > 12) {
      $.getJSON('http://localhost:8080/filmperson/list.json', function(result) {
        	  ...
        	  $.getJSON('http://localhost:8080/movie/list.json', function(result) {
                ...
                for (var k = 0; k < movieList.length; k++) {
                  ...
                  alert('aleady exists jQuery.inArray')
                      ...
                      alert('movie add succss!2')
                    }, 'json')
        	  			} // 개봉영화 db에 저장. 단, 테이블에 중복된 데이터가 없는 경우에.
      		      }

                alert('cast 단계! : ' + movieCodeArr[0] + ' : ' + $('#chrome-hidden-mno').val() + ' : ' + peopleCd)
        				$.post('http://localhost:8080/castmember/add.json', {
        					'mvNo': movieCodeArr[0],
        					'fpCode': peopleCd,
