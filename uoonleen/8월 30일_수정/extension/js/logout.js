...
$(document).on('ready', function(){
  $('#logout-btn').on('click', function() {
    ...
    $.getJSON('http://192.168.0.69:8080/auth/logout.json', function(result) {
...
$(document).on('ready', function(){
  ...
  $('#enrole-btn').on('click', function() {
    chrome.windows.create({url: "local.html", type: "popup", width:325, height:450, left:1580, top:100}, function() {
    })
  })
  ...
  */
  $('#list-btn').on('click', function() {
    ...
    if (count % 2 == 0) {
      ...
      $.getJSON('http://192.168.0.69:8080/movieperson/list.json', { mno : mno }, function(result) {

        var list = result.data.list

        for(var i = 0; i < list.length; i++) {
          $('<img>').attr('src', 'http://192.168.0.69:8080/web/movieperson/' + list[i].mpPhotopath)
        ...
        $('.profile').click(function() {
            chrome.windows.create({url: "write.html?mpNo=" + $(this).attr('data-no'), type: "popup", width:325, height:550, left:1580, top:100}, function() {})
        })
