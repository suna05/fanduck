...
/*$(document.body).on('click', '.detail-link', function(event) {
	location.href='board.html?mpNo=' + $(this).attr('data-mpNo')
...
*/
/*$(document.body).on('click', '.detail-link', function(event) {
	location.href='view.html?mpNo=' + $(this).attr('data-no')
	event.preventDefault()
})*/
...
$.getJSON('/auth/userinfo.json', function(result) {
    if (result.data) {
      ...
      $.getJSON('../../movieperson/detail.json', {'mno':mno, 'mpNo':mpNo }, 
	    function(result) {
    	    let mpProfile = $('#fileupload-personinfo')
	        let templateFn = Handlebars.compile($('#fileupload-personInfo-template').text())
	  	    let generatedHTML = templateFn(result.data) // 템플릿 함수에 데이터를 넣고 HTML을 생성한다.
	  	  console.log(generatedHTML)
	  	  console.log('나와라 ' + mpProfile)
	  	  mpProfile.text('') // tbody의 기존 tr 태그들을 지우고
	  	  mpProfile.html(generatedHTML) // 새 tr 태그들로 설정한다.
	  	  console.log(mpProfile.html())
 	   }) // getJSON()
